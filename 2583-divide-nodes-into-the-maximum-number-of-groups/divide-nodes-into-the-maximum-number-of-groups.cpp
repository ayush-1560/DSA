...
public:
    ...
    int bfs(unordered_map<int, vector<int>> &adj,int currNode, int n){
     ...
     int getMax(unordered_map<int, vector<int>> &adj, int curr, vector<bool>& visited, vector<int>& levels) {
        int maxGrp = levels[curr];
        visited[curr] = true;

        for(int &ngbr : adj[curr]) {
            if(!visited[ngbr]) {
                maxGrp = max(maxGrp, getMax(adj, ngbr, visited, levels));
            }
        }
    ...
    int magnificentSets(int n, vector<vector<int>>& edges) {
        ...
        for(int i=0;i<n;i++){
            if(colors[i]==-1){
                if(isBipartite(adj,i,colors,1)==false){
                    return -1;
                }
            }
        }
